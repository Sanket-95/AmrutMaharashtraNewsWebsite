...
// -------------------------------
// STEP 2: Send notifications for each article
// No need to fetch subscribers — OneSignal targets "All" automatically
// -------------------------------
...
foreach ($news_articles as $index => $article) {
    ...
    // 10 second delay between notifications (skip for first)
    if ($index > 0) {
    ...
    $payload = json_encode([
        "app_id"            => $APP_ID,
        "included_segments" => ["All"],       // Targets ALL subscribers directly
        "headings"          => ["en" => $title],
        "contents"          => ["en" => $summary],
        "url"               => $news_url
    ]);
    ...
    curl_setopt($ch, CURLOPT_HTTPHEADER, [
        "Authorization: Key {$API_KEY}",      // Correct format for os_v2_app_ keys
...
// -------------------------------
// STEP 3: Update onsignal flag for sent articles
// -------------------------------
