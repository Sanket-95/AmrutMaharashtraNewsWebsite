...
// Fetch news based on status and user role with filters
function fetchNews($conn, $status, $user_roll, $user_region, $district_filter, $category_filter, $from_date_filter, $to_date_filter) {
    $news = [];
    
    // DEBUG: Check parameters
    // echo "Debug: status=$status, user_roll=$user_roll, user_region=$user_region<br>";
    // echo "Debug: district_filter=$district_filter, category_filter=$category_filter, from_date_filter=$from_date_filter, to_date_filter=$to_date_filter<br>";
    ...
    if ($user_roll === 'admin' || $user_roll === 'Admin') {
        ...
        $base_query = "SELECT * FROM news_articles WHERE is_approved = ?";
        $params[] = $status;
        $param_types = "i";
    } elseif ($user_roll === 'division_head') {
        // Division head can see news from their region only
        $base_query = "SELECT * FROM news_articles WHERE Region = ? AND is_approved = ?";
        $params[] = $user_region;
        $params[] = $status;
        $param_types = "si";
    }
    ...
    if (!empty($from_date_filter)) {
        $base_query .= " AND DATE(published_date) >= ?";
    ...
    // echo "Debug: Query = $base_query<br>";
    // echo "Debug: Params = " . print_r($params, true) . "<br>";
    // echo "Debug: Param types = $param_types<br>";
...
// Get news count for each status with filters
function getNewsCounts($conn, $user_roll, $user_region, $district_filter, $category_filter, $from_date_filter, $to_date_filter) {
    $counts = [
        'pending' => 0,
        'approved' => 0,
        'disapproved' => 0
    ...
    // Build base query and parameters for each status
    $statuses = [0, 1, 2];
    ...
    foreach ($statuses as $status) {
        ...
        if ($user_roll === 'admin' || $user_roll === 'Admin') {
            $base_query = "SELECT COUNT(*) as count FROM news_articles WHERE is_approved = ?";
            $params[] = $status;
            $param_types = "i";
        } elseif ($user_roll === 'division_head') {
            $base_query = "SELECT COUNT(*) as count FROM news_articles WHERE Region = ? AND is_approved = ?";
            $params[] = $user_region;
            $params[] = $status;
            $param_types = "si";
        }
...
$news_items = fetchNews($conn, $current_status, $user_roll, $user_region, $district_filter, $category_filter, $from_date_filter, $to_date_filter);
$news_counts = getNewsCounts($conn, $user_roll, $user_region, $district_filter, $category_filter, $from_date_filter, $to_date_filter);
...
$active_filters = [];
if (!empty($district_filter)) {
    $active_filters[] = 'जिल्हा: ' . getMarathiDistrictName($district_filter);
}
if (!empty($category_filter)) {
    $active_filters[] = 'वर्ग: ' . getMarathiCategoryName($category_filter);
}
if (!empty($from_date_filter)) {
    $active_filters[] = 'पासून: ' . date('d-m-Y', strtotime($from_date_filter));
}
if (!empty($to_date_filter)) {
    $active_filters[] = 'पर्यंत: ' . date('d-m-Y', strtotime($to_date_filter));
}
?>
...
<div class="container mt-4 mb-5">
    ...
    <div class="card shadow-sm mb-4" style="border: 2px solid #4CAF50; border-radius: 10px;">
        <div class="card-body p-3">
            <div class="d-flex align-items-center">
                <i class="fas fa-filter me-2" style="color: #4CAF50; font-size: 20px;"></i>
                <h6 class="mb-0 me-3" style="color: #4CAF50; font-family: 'Khand', sans-serif;">सक्रिय गाळणी:</h6>
                <div class="d-flex flex-wrap gap-2">
                    <?php foreach ($active_filters as $filter): ?>
                        <span class="badge rounded-pill px-3 py-2" 
                              style="background: linear-gradient(135deg, #4CAF50, #2E7D32); color: white; font-family: 'Mukta', sans-serif;">
                            <?php echo $filter; ?>
                        </span>
                    <?php endforeach; ?>
                    <a href="?status=<?php echo $status_filter; ?>" 
                       ...
                       style="font-family: 'Mukta', sans-serif;">
                        <i class="fas fa-times me-1"></i> गाळणी रद्द करा
                    </a>
                </div>
            </div>
        </div>
    ...
    <div class="card shadow-sm mb-4" style="border: 2px solid #FF6600; border-radius: 10px;">
        <div class="card-body p-3">
            <div class="d-flex flex-column flex-md-row gap-3 justify-content-center">
                <!-- Pending Tab -->
                <?php
                $pending_url = "?status=pending";
                if (!empty($district_filter)) $pending_url .= "&district=" . urlencode($district_filter);
                if (!empty($category_filter)) $pending_url .= "&category=" . urlencode($category_filter);
                if (!empty($from_date_filter)) $pending_url .= "&from_date=" . urlencode($from_date_filter);
                if (!empty($to_date_filter)) $pending_url .= "&to_date=" . urlencode($to_date_filter);
                ?>
                <a href="<?php echo $pending_url; ?>" class="btn btn-lg shadow position-relative <?php echo $status_filter === 'pending' ? 'active' : ''; ?>" 
                ...
                <!-- Approved Tab -->
                <?php
                $approved_url = "?status=approved";
                if (!empty($district_filter)) $approved_url .= "&district=" . urlencode($district_filter);
                if (!empty($category_filter)) $approved_url .= "&category=" . urlencode($category_filter);
                if (!empty($from_date_filter)) $approved_url .= "&from_date=" . urlencode($from_date_filter);
                if (!empty($to_date_filter)) $approved_url .= "&to_date=" . urlencode($to_date_filter);
                ?>
                <a href="<?php echo $approved_url; ?>" class="btn btn-lg shadow position-relative <?php echo $status_filter === 'approved' ? 'active' : ''; ?>" 
                ...
                <!-- Disapproved Tab -->
                <?php
                $disapproved_url = "?status=disapproved";
                if (!empty($district_filter)) $disapproved_url .= "&district=" . urlencode($district_filter);
                if (!empty($category_filter)) $disapproved_url .= "&category=" . urlencode($category_filter);
                if (!empty($from_date_filter)) $disapproved_url .= "&from_date=" . urlencode($from_date_filter);
                if (!empty($to_date_filter)) $disapproved_url .= "&to_date=" . urlencode($to_date_filter);
                ?>
                <a href="<?php echo $disapproved_url; ?>" class="btn btn-lg shadow position-relative <?php echo $status_filter === 'disapproved' ? 'active' : ''; ?>" 
        ...
        <div class="text-center py-5">
            ...
            <h4 style="color: #FF6600; font-family: 'Khand', sans-serif;">
                ...
                if (!empty($active_filters)) {
                    echo 'गाळणीनुसार कोणत्याही ' . getMarathiStatusName($current_status) . ' बातम्या नाहीत';
                } else {
                    echo 'कोणत्याही ' . getMarathiStatusName($current_status) . ' बातम्या नाहीत';
                }
                ?>
            </h4>
                            ...
                            <div class="d-flex justify-content-between align-items-center">
                                ...
                                <small class="text-muted" style="font-family: 'Mukta', sans-serif;">
                                    ...
                                    if (!empty($news['published_date'])) {
                                        echo date('d-m-Y', strtotime($news['published_date']));
                                    } else {
